...
train_cm_map_df <- cbind(Train, trainProbs) %>%
  mutate(confusionMtx = case_when(predClass == 1 & obs == 1 ~ "True Positive",
...
```{r}
pgh_inundationProbs <- predict(floodModel, pgh_PredictionFishnet, type="response")

hist(pgh_inundationProbs)

pgh_inundationProbs <- data.frame(pgh_inundationProbs)
pgh_inundationProbs$predClass  = ifelse(pgh_inundationProbs$pgh_inundationProbs > .28 ,1,0)
```
